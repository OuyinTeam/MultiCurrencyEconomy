...
object BackupService {
     ...
     * @return true = 回滚成功
     */
    fun rollbackPlayer(snapshotId: String, playerUuid: String): Boolean {
        if (!DatabaseManager.ready) return false

        val snapshots = BackupRepository.findBySnapshotIdAndPlayer(snapshotId, playerUuid)
        if (snapshots.isEmpty()) {
            warning("[MCE] 在快照 $snapshotId 中找不到玩家 $playerUuid 的数据。")
            return false
        }
        ...
        try {
            snapshots.forEach { snapshot ->
                ...
                val account = AccountRepository.getOrCreate(
                    snapshot.playerUuid, snapshot.playerName, snapshot.currencyId
                )
                ...
                // 更新缓存
                AccountService.loadPlayerBalances(snapshot.playerUuid, snapshot.playerName)
            }
